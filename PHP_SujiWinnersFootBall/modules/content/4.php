...
<main>
  <section class="bg-white" id="sub1">
    <form id="sangdam" action="/consulting_upload.php" method="post">
      <div>
        <span>상담자 이름</span>
        <div><input type="text" name="name" value="" onKeyup="this.value=this.value.replace(/^[가-힣]{2,4}|[a-zA-Z]{2,10}\s[a-zA-Z]{2,10}$/;" placeholder="성함"></div>
      </div>
      <div>
        <span>핸드폰 번호</span>
        <div><input type="tel" name="phone" value="" onKeyup="this.value=this.value.replace(/[^0-9]/g,'');" placeholder=" - 없이 입력해주세요"></div>
      </div>
      <div>
        <span>상담 내용</span>
  ...
  <div id="popuplayer">
    <div>
      ...
      <section>
        <p>개인정보 취급 방침 동의</p>
        <p>개인정보의 수집 및 이용목적</p>
        <p>성명, 주소, 연락처 :  위너스 풋볼아카데미의 안내나 권유 등 서비스 이용 안내, 마케팅 메시지 전송을 활용한 홍보활동 동의.</p>
