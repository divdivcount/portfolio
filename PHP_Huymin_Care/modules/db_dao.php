	...
	*/
	...
	public function SelectPageList($cPage, $viewLen, $select = '*', $where = null) {
		...
		*/
		...
		$this->openDB();
		if($where) $query = $this->db->prepare("select $select from $this->quTable where $where order by $this->quTableId desc limit :start, :end");
		else $query = $this->db->prepare("select $select from $this->quTable order by $this->quTableId desc limit :start, :end");
